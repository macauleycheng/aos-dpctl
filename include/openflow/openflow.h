...
/* when change the cpnt, must modify the file
 * \oflib\oxm-match.def
 */
#define CONFIG_ACCTON_EXP 0
#define CONFIG_OFDPA_EXP 1
...
/* begin AR#exp */
enum ATN_oxm_ofb_match_exp_fields {
    ATN_OFPXMT_OFB_INNER_ETH_DST    = 0,
    ATN_OFPXMT_OFB_INNER_ETH_SRC    = 1,
    ATN_OFPXMT_OFB_INNER_VLAN_VID   = 2
};
/* end AR#exp */
...
{
    OFDPA_OFPXMT_OFB_VRF = 1,
    OFDPA_OFPXMT_OFB_TRAFFIC_CLASS = 2,
    OFDPA_OFPXMT_OFB_COLOR = 3,
    OFDPA_OFPXMT_OFB_DEI = 4,
    OFDPA_OFPXMT_OFB_QOS_INDEX = 5,
    ...
    OFDPA_OFPXMT_OFB_MPLS_NEXT_LABEL_IS_GAL = 13,
    OFDPA_OFPXMT_OFB_OAM_Y1731_MDL = 14,
    OFDPA_OFPXMT_OFB_OAM_Y1731_OPCODE = 15,
    OFDPA_OFPXMT_OFB_COLOR_ACTIONS_INDEX = 16,
    OFDPA_OFPXMT_OFB_TXFCL = 17,
...
/* begin AR#exp */
enum ATN_ofp_exp_action_exp_type {
    ATN_OFPAT_PUSH_L2_GRE_HEADER = 0,        /* push L2 GRE header. */
    ATN_OFPAT_POP_L2_GRE_HEADER = 1,        /* pop L2 GRE Header. */
};
/* end AR#exp */
...
/* For OFDPA2.0
 */
struct OFDPA_ofp_action_exp {
    struct ofp_action_experimenter_header header;
    uint16_t exp_type;
    uint8_t pad[6]; /* OXM TLV - Make compiler happy */
};
OFP_ASSERT(sizeof(struct OFDPA_ofp_action_exp) == 16);

struct OFDPA_ofp_action_exp_set_u8 {
    struct ofp_action_experimenter_header header;
    uint16_t exp_type;
    uint8_t value;
    uint8_t pad[5]; /* OXM TLV - Make compiler happy */
};

#pragma pack(1)
struct OFDPA_ofp_action_exp_set_u32 {
    struct ofp_action_experimenter_header header;
    uint16_t exp_type;
    uint32_t value;
    uint8_t pad[2]; /* OXM TLV - Make compiler happy */
};

struct OFDPA_ofp_action_exp_drop_status {
    struct ofp_action_experimenter_header header;
    uint16_t exp_type;
    uint32_t drop_index;
    uint8_t drop_type;
    uint8_t pad[1]; /* OXM TLV - Make compiler happy */
};
#pragma pack()
/* End of OFDPA2.0
 */
...
struct ofp_meter_band_experimenter_color_set {
    uint16_t   type;   /* OFPMBT_EXPERIMENTER (0xFFFF) */
    uint16_t   len;    /* Length in bytes of this band. */
    uint32_t   rate;   /* Rate for this band. */
    uint32_t   burst_size; /* Size of bursts. */
    uint32_t   experimenter; /* Experimenter Id: (00-00-10-18) */
    uint16_t   exp_type;     /* OFPMBT_COLOR_SET (3) */
    uint8_t   mode;     /* OFDPA_COLOR_SET_BAND_MODE_*  */
    uint8_t   color_aware;   /* Color-blind (0) or color-aware (1) */
    uint8_t   color;     /* New color, one of Yellow(1), Red(2) */
