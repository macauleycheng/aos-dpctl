...
str_to_ipv6(const char *str_, struct in6_addr *addrp, struct in6_addr *maskp)
{
    ...
    if (netmask) {
        /*check netmask is number or address format*/
        if(inet_pton(AF_INET6, netmask, &net) == 1){
                *maskp = ipv6_addr_bitand(&net, &in6addr_exact);
        ...
        else
        {
            ...
            if (prefix < 0 || prefix > 128) {
                printf("%s: network prefix bits not between 0 and 128\n",
                          str);
                return -1;
            } else {
